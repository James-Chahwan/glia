...
set -u
LOG=/home/ivy/Code/glia/scratch/lens/cycle/cycle_3_holdout_launch.log
...
$(python3 -c "import json; [print(f'- {x[\"instance_id\"]}') for x in json.load(open('/home/ivy/Code/glia/scratch/lens/manifests/holdout.json'))]")

EOF

phase "Cycle 3.0 holdout"
for INST in $(python3 -c "import json; [print(x['instance_id']) for x in json.load(open('/home/ivy/Code/glia/scratch/lens/manifests/holdout.json'))]"); do
...
"; then
  ...
  rm -rf /home/ivy/Code/glia/scratch/latent/out/inst-${INST}-14b-q4-3.0-holdout 2>/dev/null
