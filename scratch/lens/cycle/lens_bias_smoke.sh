...
#
# Output: scratch/lens/cycle/lens_bias_alpha_sweep.jsonl

set -u
SMOKE_DIR=/home/ivy/Code/glia/scratch/lens/cycle/lens-bias-smoke
mkdir -p "${SMOKE_DIR}"
...
INSTANCE_DIR=$(ls -td /home/ivy/Code/glia/scratch/latent/out/inst-marshmallow-code__marshmallow-1359-*-cycle-2.0-gpu-14b 2>/dev/null | head -1)
...
for ALPHA in 0.1 0.3 0.5 1.0 2.0; do
  echo "  alpha=${ALPHA}"
  /home/ivy/Code/glia/scratch/lens/target/release/lens-attention-bias \
    --weights "${GGUF}" \
    --tokenizer "${TOKENIZER}" \
    --prefix "${PREFIX}" \
    --suffix "${SUFFIX}" \
