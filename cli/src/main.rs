...
enum Cmd {
    ...
    /// Walk a repo and write one `engram_core::Gmap` bincode file (engram's
    /// Path-A seed) plus a `<out>.files.json` span sidecar. Separate from the
    /// native `.gmap` — this is the cross-engine contract format.
    ExportEngram {
        /// Path to the repo root.
        repo: String,
        ...
        #[arg(long)]
        out: Option<String>,
        /// Keep substrate-only synthetic nodes (npm deps, event names,
        /// generated stubs) that are filtered from the export by default.
        #[arg(long)]
        include_noise: bool,
...
fn main() {
    ...
    let exit = match cli.cmd {
        ...
        Cmd::ExportEngram {
            ...
            include_noise,
            exclude,
        } => cmd_export_engram(&repo, out.as_deref(), include_noise, exclude),
        Cmd::InstallHooks {
            repo,
            uninstall,
            command,
...
// ----------------------------------------------------------------------------
// `export-engram` — write the engram_core::Gmap contract file + span sidecar
// ----------------------------------------------------------------------------

fn cmd_export_engram(
    repo: &str,
    out: Option<&str>,
    include_noise: bool,
    exclude: Vec<String>,
    ...
    let opts = repo_graph_engram_export::ExportOptions {
        include_noise,
        exclude,
    };
    let stats = match repo_graph_engram_export::export_engram_gmap(
        &result.merged,
        repo_root,
        &out_path,
        &opts,
    ...
    eprintln!(
        ...
        stats.files,
        out_path.display(),
        repo_graph_engram_export::sidecar_path(&out_path).display(),
    );
    ...
    if stats.dropped_noise > 0 || stats.dropped_excluded > 0 {
        eprintln!(
            ...
            stats.dropped_noise,
            if include_noise { " (kept: --include-noise)" } else { "" },
