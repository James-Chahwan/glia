...
- Replaces `run_llama_pathB.py` with a Rust binary (`synth_generate.rs` or extend `bench/lens/` to do full generation, not just 2 forward passes).
...
- Scope: substantial. bench/lens/ already proves the FFI works; needs greedy/sample loop + chat template + EOS detection + the embedding-pool splicing. ~6-10 hours of Rust.
